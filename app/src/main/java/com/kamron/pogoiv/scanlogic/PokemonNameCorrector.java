...
public class PokemonNameCorrector {
    ...
    public PokeDist getPossiblePokemon(String poketext, String candytext, Optional<Integer> candyUpgradeCost,
        ...
        if (normalizedCandyName.contains(StringUtils.normalize(pokeInfoCalculator.get(182).name))
                && (candyUpgradeCost.isPresent())){ //its not an azumarill
        ...
        try {
            switch (guess.pokemon.number) {
                ...
                case (412): // Wormadam
                    ...
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.GROUND))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(1), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.STEEL))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(2), 0);
                    ...
                    break;
                case (478): // Rotom
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.GHOST))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(0), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.ICE))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(1), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.FLYING))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(2), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.GRASS))) {
                ...
                case (492): // Arceus
                    ...
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.FIGHTING))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(1), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.FLYING))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(2), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.POISON))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(3), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.GROUND))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(4), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.ROCK))) {
                        return new PokeDist(pokeInfoCalculator.get(guess.pokemon.number).forms.get(5), 0);
                    }
                    if (normalizedPokemonType.contains(normalizePokemonType(Type.BUG))) {
